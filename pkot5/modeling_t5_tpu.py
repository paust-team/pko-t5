...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if position_bias is None:
            ...
            if mask is not None:
                logger.debug(f"rank={xm.get_ordinal()} position_bias={position_bias.shape} mask={mask.shape}")
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens, device_map: Optional[Dict[int, List[int]]] = None):
        ...
        if self.model_parallel:
            ...
            block: List[nn.Module] = [nn.Identity() for _ in range(config.num_layers)]
    ...
    def _forward_layer(
            self, idx, layer_module, hidden_states,
            attention_mask=None,
            position_bias=None,
            encoder_hidden_states=None,
            encoder_attention_mask=None,
    ...
    def forward(self,
        ...
        logger.debug(f"input_ids=({input_ids.shape}, {input_ids.device}, {input_ids.dtype})")
        ...
        logger.debug(f"extended_attention_mask={extended_attention_mask.shape}")
        ...
        _forward_layer = functools.partial(
            self._forward_layer,
        ...
        if not self.model_parallel:
            for i, layer_module in enumerate(self.block):
                hidden_states = _forward_layer(i, layer_module, hidden_states)
            ...
            for dev in devices:
                if dev == rank:
                    for layer_num in self.device_map[dev]:
                        layer_module = self.block[layer_num]
                        logger.debug(f"rank={xm.get_ordinal()} hidden_states={hidden_states.shape}")
                        hidden_states = _forward_layer(layer_num, layer_module, hidden_states)
                    if dev == self.last_device:
                        logger.debug(f"Run T5Stack on last device rank={rank}")
                        hidden_states = self.final_layer_norm(hidden_states)
                        hidden_states = self.dropout(hidden_states)
                hidden_states = xm.collective_permute(hidden_states, [[i, (i + 1) % (self.last_device + 1)] for i in range(self.last_device + 1)])
...
#         self.device_map = (
#             get_device_map(len(self.block), range(1, 9)) if device_map is None else device_map
#         )
#         assert_device_map(self.device_map, len(self.block))
#         self.model_parallel = True
#         self.first_device = "cpu" if "cpu" in self.device_map.keys() else xm.xla_device(min(self.device_map.keys()))
#         self.last_device = xm.xla_device(max(self.device_map.keys()))
#         # Load onto devices
#         for k, v in self.device_map.items():
#             for layer in v:
#                 self.block[layer] = self.block[layer].to(xm.xla_device(k))
#
...
#                 for k, v in self.device_map.items():
#                     if i == v[-1] and k != 8:
#                         # xm.collective_permute([[k, k + 1]])
#                         hidden_states = hidden_states.to(xm.xla_device(k + 1))
#
#         hidden_states = self.final_layer_norm(hidden_states)
#         hidden_states = self.dropout(hidden_states)
#
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        logger.debug(f"encoder output hidden_states={encoder_hidden_states.shape}")
